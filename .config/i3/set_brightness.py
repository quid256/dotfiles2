...
if __name__ == "__main__":
    ...
    if cmd.startswith("+") or cmd.startswith("-"):
        ...
        b_out = r(
            'xrandr --verbose | awk \'/Brightness/ {print "B " $2} / connected/ {print "M " $1}\'',
            capture_output=True,
            shell=True,
        )
        assert (
            b_out.returncode == 0
        ), "Failed to read brightness and monitor info from xrandr"
        ...
        for m in m_list:
            assert (
                r(
                    "xrandr --output {out} --brightness {bn}".format(out=m, bn=new_b),
                    shell=True,
                ).returncode
                == 0
            )
